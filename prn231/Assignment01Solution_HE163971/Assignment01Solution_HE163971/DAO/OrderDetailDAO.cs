...
namespace Assignment01Solution_HE163971.DAO
{
    public class OrderDetailDAO
    {
        ...
        public static OrderDetail FindOrderDetailById(int proId, int odId)
        {
            ...
            try
            {
                using (var context = new MyDbContext())
                {
                    list = context.OrderDetails.SingleOrDefault(x => x.ProductId ==proId && x.OrderId == odId);
        ...
        public static void DeleteOrderDetail(OrderDetail p)
        {
            ...
            try
            {
                using (var context = new MyDbContext())
                {
                   var list = context.OrderDetails.SingleOrDefault(x => x.ProductId == p.ProductId && x.OrderId == p.OrderId);
